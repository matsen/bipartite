...
- **When the document asks someone to DECIDE, lead with the decisive reason, not the one you measured.** ⚠ The rule above covers a document describing a *build*; a policy case, a ruling request or a design argument has no deliverable, only reasons, and nothing here says how to order those. ⛔ **Author confidence tracks how much CONTEXT the author has, not how much WEIGHT the argument carries for a reader — so it is not a proxy for decisiveness, and ordering by it orders by a property the reader cannot see.** ➡ **The check is mechanical and does not require agreeing with any of that: for each reason, ask whether it would still decide the question if the reader rejected every other one. Lead with the ones that survive; the rest are corroboration and should be labelled as such.** ⭐ The generator, which explains why this keeps happening rather than establishing the rule: **the argument you measured yourself is the one you have the most context for, so it feels the most solid** — exactly backwards from the weight it carries for a reader who was not there. ⚠ Measured 2026-09-19 on `matsengrp/phyz`, and the author named it better than this rule does: a worker arguing that Snakefile-less experiment directories are an allowed shape led with *"77 of 304 directories are already that shape"*, which it had counted itself, and buried as a parenthetical the fact that the one such directory dated at the exemption cutoff would turn `make check-fast` red on unmodified `main` — **the only reason that decides the question rather than merely supporting it.** Its own account: ***"Having measured something is not the same as it being the load-bearing argument, and I ordered by the former."***
...
- **KEEP THE BRANCHES OF A CONDITIONAL WITHIN A READER'S EYEFUL OF EACH OTHER.** A reader — or a `grep` — that lands on one branch of an enumeration reads it as an assertion, because nothing adjacent contradicts it. ⚠ **Measured 2026-09-21 on `skills/bip-conductor-spawn/SKILL.md:1378-1379`: one correctly-written two-branch conditional produced two OPPOSITE false conclusions from two careful readers within one hour.** One grepped a single file, found nothing, and reported the claim absent from the repo. The other grepped the tree, found `:1378` — *"**A real copy** (from `go install`). Then a landed commit changes nothing until someone reinstalls"* — and read it as a surviving stale claim, when it is the first of two branches with the second on the next line. ⛔ **Neither instrument could have caught the other's failure: a miss cannot reveal a false positive, and the hit would have caught the miss and then mis-ruled it.**

  ⭐ **What made both errors recoverable was structural, not textual — the two branches sit on adjacent lines**, so one author caught itself on re-read and the other's conclusion survived its bad evidence. Split across distant paragraphs, both would have stood. ➡ **So the remedy is adjacency, not simplification.** The temptation is to conclude the prose should be flatter; that is wrong here, because the conditional is load-bearing and flattening it reinstates the false claim it replaced. ⚠ **Note the uncomfortable direction this points: a file dense with correctly-scoped conditionals is MORE grep-hostile, not less — careful writing attracts this failure.** The two reading-side rules are the other half, and they do not imply each other: **a grep hit inside an enumeration is a branch, not a claim; a grep miss is a statement about the files searched, not about the repo.**
...
- Buried decisive reason (the argument that would settle it alone appears after ones that would not, or as a parenthetical).
