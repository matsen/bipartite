...
Many cold-start and poll skills (`/bip-ms`, `/bip-ms-poll`, `/bip-epic`, `/bip-conductor`, `/bip-ms-audit`) scan many independent sources — every tracked EPIC, every active clone, every recently merged PR. Reading those bodies directly into the primary agent burns context: a single `/bip-ms` invocation can spend 85K tokens loading EPIC bodies and PR descriptions that the primary doesn't need verbatim.
