...
➡ ⛔ **So, before transmitting a lesson to a worker, check whether the skill already states it. If it does, the transmission is at best redundant and at worst a bypass** — a skill states its lesson *inside the procedure that also does everything else*; a message states it *detached from that procedure*. ⚠ **A worker cannot tell that your correct advice is a subset of a path it was already on.**
