---
name: issue-update
description: Re-evaluate an existing GitHub issue against current repo state, fix stale content, then run /issue-check
...
PRs, outdated flag names, superseded designs), then run `/issue-check`
for mechanical completeness.

This is the "think, then check" complement to `/issue-check`. Use
`/issue-check` for a new issue that hasn't been filed yet. Use
`/issue-update` for an existing issue that may have drifted from
reality.

## Usage

```
/issue-update org/repo#N
/issue-update 123            # uses current repo context
```
...
This is the key step that distinguishes `/issue-update` from
`/issue-check`. Examine the real world, not just the issue text.
...
- **Domain fixes**: Architectural changes, validation additions, scope
  adjustments
- **Mechanical fixes**: From `/issue-check` (grouped by severity)
