...
If the ancestor test fails (the base moved, e.g. mid-batch), the default is to rebase `refs/pr/<N>` onto `origin/<base>` in a scratch worktree, `git push --force-with-lease origin HEAD:<headRefName>`, wait for CI (Step 5.5), and re-run the guard. Merging a stale head without the rebase is your call when you can say why it is safe (e.g. its files are disjoint from what landed since it branched); say so in a PR comment.
