...
⛔ **IT DOES NOT FAIL TO FIRE. IT FIRES AND NAMES THE WRONG ISSUE**, so a reader sees `#2`, finds it nonexistent or ancient, and **dismisses a true positive.** **That is a counterexample to `EVIDENCE-DISCIPLINE.md`'s own asymmetry, not an instance of it**: that file says a wrong NON-ZERO is *"visible on inspection"* while a wrong zero closes the question. Here the wrong non-zero is visible and dismissed anyway, because `2` reads as noise rather than as a value. **A wrong non-zero is only self-correcting when it is implausible; one that looks like debris is not.**

⚠ **And "the `closingIssuesReferences` check below is authoritative anyway" is a reason this was survivable, NOT a reason to leave it: a reader who learns the grep lies stops running the STEP, not just the grep — including the authoritative check sitting beside it.** A gate that is both redundant and wrong is worse than either alone. ⚠ *That reader-behaviour claim is reasoning, not a measurement — nobody has observed it; the pin below stands on the truncation itself.*

⭐ **Why the 29-row table below could not catch it, which is the generalisable half: every pre-existing row used a SUFFIXED keyword (`Closes`, `Fixes`, `Resolves:`), because that is what everyone writes.** The fixture set was drawn from the same habit as the code, so the table was green on a machine where three of GitHub's nine keywords were broken. ➡ **GitHub's closing keywords are a CLOSED SET OF NINE. There is now one row per keyword, so the table is complete BY CONSTRUCTION rather than green by what it happens to contain** — and the next engine change is caught mechanically instead of by someone noticing.
...
**Three workers hit this contradiction independently on 2026-09-22 — PRs #2909, #2912 and #2920, all merged, spanning four hours — and all three refused the step and flagged it rather than arbitrating** — which is what this repo's own rule asks for (*"when a rail and an instruction conflict, the conflict is a defect in the pair, not a puzzle for the worker to solve"*). **The pair stayed broken through all three refusals, which is the argument for fixing the document rather than commending the workers.** ⚠ **Three is a count, not a rate, and it is stated that way deliberately.** A draft of this entry carried it as *"every landing that reached this step"* on the reasoning that the denominator was fleet knowledge a reader could not check. **It was checkable and it refutes the rate:** this skill posts a `🤖 ... worklog preserved` pointer comment, which is a public marker of having run, and **8 of the 10 PRs merged on `matsengrp/phyz` that day carry it** against a numerator of 3. One of the other five — a worker landing from a pooled clone, the same one that flagged this on an earlier PR — did not mention the step at all, and what happened there is unknown. ⛔ **So skipping is not established as the normal path; three workers hit the contradiction and refused, which is sufficient on its own.** ⭐ **Both parties then spent real effort establishing a rate the argument never needed — an internal contradiction is not made more contradictory by a frequency. Check whether your case is already closed before gathering evidence for it.**
