...
```
/bip-plan
/bip-plan "focus on the serialization bug we discussed"
```
...
1. Run `/bip-pr-review` to catch quality issues before merging
2. Address any blocking findings from the review
3. **Stop and report to the user** — do not merge. The user will run `/bip-pr-land` explicitly when ready.
```
