...
- **And remote branches are not a usable substitute.** Under squash-merge every historical branch stays permanently ahead of `main`, so "ahead of main" does not discriminate live from long-dead: that repo had **847** remote branches when this was measured (2026-09-14; **819** a few hours later — the count drifts as merged branches are deleted, so treat the magnitude, not the figure, as the claim) and every sampled branch is ahead of `main`. Intersecting against them is noise, not coverage.
