...
**The split has a diagnostic function beyond the division of labour, and the argument for it — the mechanism, its limit, and the measured day behind it — is `/bip-conductor` under "The fleet/topic line."** Read it there; it is not restated here, because the two copies had already drifted apart by the time anyone noticed.

What that argument asks of *this* side, and all of it is cheap:

- **Send raw measurements rather than conclusions**, and re-derive a peer's number before acting on it.
- **Re-derive silently and report only the delta.** A peer's five-item list that turns out to have nine is worth one line, not a second copy of their reasoning. Neither practice licenses re-narrating the peer's analysis to the user — that is the duplication `/bip-conductor`'s fleet/topic rule forbids ("consume it as a constraint, log it, and do not re-verify, re-narrate, or re-litigate it").
- **Verify the observation a diagnosis explains, before the explanation.** A diagnosis does not merely explain an observation — it restates it, more narrowly, and the restatement carries the smuggled premise while the explanation gets all the scrutiny. This is the one limit the split does not cover: a premise both sessions accepted is in neither differential, so no amount of cross-reading reaches it. You cannot close it yourself either — the party holding the observation is the runner, and Step 7 routes every worker message through the conductor. **So name the observation the diagnosis assumes, and ask the conductor to put that question to whoever ran it.** Argument and measured case: `/bip-conductor`, "The fleet/topic line."
- **Keep corrections low-ceremony.** "That framing is wrong, here is why" in one line, no preamble and no apology round, in either direction. A correction that costs a diplomatic round trip does not get made at the margin, and the marginal ones are where the value was.
...
⭐ **But ONE sub-case of it DOES have a cheap detector, and the paragraph above should not be read as covering it: when the shared thing is a NAMED SYMBOL rather than an artifact's meaning, the consumer set is one command.** An issue that edits a **shared definition** — a constant, a params list, a fixture path — has a footprint equal to that symbol's CONSUMER SET, not to the file it is defined in.

```bash
bip epic collide --symbol MATCHED_PARAMS            # add --path <subtree> to narrow it
```

⛔ **RUN IT AT BRIEF TIME AND PASTE ITS STAMP, OR DO NOT QUOTE THE NUMBER.** A blast radius is a **measurement with a date**, not a property of the constant, so the command prints the repo, the commit and the timestamp with the counts, and splits them into **code / prose / data** because those are three different kinds of work — the prose hits go stale exactly like the code hits but are a separate job, and the data hits are committed results artifacts that must **not** be edited. ⚠ **`matsengrp/phyz`, measured: 26 consumers at `47a35e7c` — 17 code plus 9 READMEs — became 39 at `ede049ec`, three landings and ~14 hours later, and all 13 new entries came from ONE of them: the PR whose own thesis was that this blast radius made the edit expensive. It grew the radius by 46%.** *"26 consumers"* tells a worker one kind of work where *"17 code plus 9 prose"* tells it two.

⛔ **AND IT CANNOT SEE THE INDIRECT CONSUMERS — files that import the defining module and never name the symbol — so it says so on its own last line rather than leaving you to remember.** Measured at those same three commits: **14, unchanged by any of them**, while the direct count moved 26 → 39. ⚠ **A worker who greps gets the direct set and believes it has the set.**

⭐ **The command replaces the COUNTING, not the READING, and the two instances that produced this rule are both invisible to a count** — a path-overlap check called all three consumers disjoint, **correctly, which is the point.** A second experiment's `common2830.py` *imports* the list and appends a value the proposed edit would ALSO add, giving a duplicated key that **the tool accepts at exit 0 with no warning** — nothing fails loudly; the key just has two definitions that can drift. And a **live worker's unlanded branch** partitions the list by string prefix, so a new entry lands in one partition or the other by its name, with no membership check. ➡ **Run the command for the population; then read the hits.**
...
⛔ **AND THE MOMENT YOU WILL SKIP IT IS PREDICTABLE, WHICH IS THE ONLY USEFUL THING HERE: you re-check when you are about to ACT on the state — approve, rule, spawn — and recite from memory when you are merely REPORTING it.** ⭐ **Reporting IS acting, one party removed**, because a reported state is acted on by whoever reads the report. Observed 2026-09-20 on `matsengrp/phyz` and volunteered by the session that did it: a conductor wrote *"hazel is holding for your approval"* **56 seconds after that approval had posted to the PR**, about an hour after shipping the very rule that made a gate's state readable from the artifact. ⚠ **A durable artifact removes the EXCUSE, not the HABIT** — the check was one command away and memory was used instead. ➡ **So run the command in the sentence that reports, not only in the one that acts.**
...
⭐ **RESOLVED 2026-09-20 BY DELIBERATE PROBE — both the UNIT and the COMMENT ceiling, which had been recorded here as untested.** **(1) THE BODY CEILING COUNTS BYTES, NOT CHARACTERS.** One discriminating write settles it, because `bytes >= chars` always: a body of **260,927 characters / 267,449 bytes** — *under* `2^18` in chars, *over* it in bytes — was **REJECTED**. **(2) THE COMMENT CEILING IS NOT `2^16`.** Comments of **65,556**, **72,072** and **131,076** characters were all ACCEPTED and read back exact; **264,058** was rejected. ⛔ **AND GITHUB'S OWN ERROR TEXT IS UNRELIABLE ABOUT BOTH THE MAGNITUDE AND THE UNIT: the rejection says *"maximum is 65536 characters"* while a 131,076-character comment had already been accepted on the same issue minutes earlier.** ➡ **Never take the limit from the message.** ⭐ **So archiving into a comment IS available and the `2^16` worry is retired — but a ~260 KB body is MARGINAL against a rejection observed at 264,058, so SPLIT IT ACROSS TWO and read each back.** ⚠ **And measure the stored body with `gh api repos/<org>/<repo>/issues/<n>` and `len(body.encode())`, NOT `gh issue view --json body -q .body | wc -c` — `jq` appends a newline, so every figure through that pipe is ONE TOO HIGH.** ⭐ **The meta-lesson, since this sat here as *untestable-until-needed* for weeks: it cost one scratch issue and six API calls. "Relocate the test to the moment of need" was the right call ONLY because someone eventually reached that moment — it is not a substitute for the test being cheap.**
