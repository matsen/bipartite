...
Measured `matsengrp/phyz` 2026-09-19, **three sessions in one afternoon on the same basis**: two PRs each reported "zero overlap" with the other and shared **five** files (`build.zig`, `CLAUDE.md`, two `docs/ml/*.md`, `experiments/README.md`), one producing a real content conflict; and a brief recorded a live neighbour as *"`experiments/` plus `src/ml/`"* when that branch also edited **both** of the briefed issue's primary files. ⛔ **The last one did not even use the neighbour's issue body — it used a memory of what the issue was ABOUT, which is a forecast of a forecast.** ⭐ The generator is ordinary and invisible from inside: an issue named *"a CLV fix"* files under `src/ml/`, and **any behaviour change to a compared axis also updates the docs that record the comparison.** ➡ **Docs, `build.zig`, a shared README and a registry TSV are where two slots on one area actually collide — and where a conflict is least likely to be noticed, because nobody re-reads a doc merge.**
