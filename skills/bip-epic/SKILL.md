...
**There is no cheap detector for the CITATION case — where the second party READS an artifact as evidence — and you should not pretend otherwise.** ⚠ **That scoping is a correction: this sentence used to claim no detector existed for the whole class, and the sub-case below falsifies it.** A file list cannot see the citation case — resolving it needs someone who knows what each artifact is *for*, which is a topic-level read at brief time. Treat that as one more reason briefs go through the epic session rather than being generated from diffs. Two questions worth asking of any issue that regenerates or replaces a committed artifact:
...
⭐ **THE SUB-CASE THAT DOES HAVE A CHEAP DETECTOR: *their new TEST pins what my change MOVES*.** The difference from the citation case is mechanical, and it is what makes this one findable: there the second party **reads** an artifact, here it **asserts a threshold on a quantity** — and a threshold names the quantity in its own source text, where a citation names only the artifact.

Measured `matsengrp/phyz` 2026-09-19: a landed PR added an effect gate asserting one `--param`'s arm does **≤80%** of the other's `ops_search_lower_clv`, on a matched argv that enables the very branch-length sweep a second open PR was rewriting — so the rewritten code ran in **both** of that test's arms. ⛔ **Both PRs' file sets had been intersected correctly and completely; the two sides share no path at all, so the intersection could not see it, and the second PR's entire routed suite went red on rebase.**

➡ **The check is not a path intersection but a grep over OBSERVABLES.** For any change that moves something others assert on — an `ops_*` counter, a pinned lnL, a wall figure, a fixture hash — search the open queue's branches for that **name** rather than diffing filenames. ⭐ **A file intersection answers "do we write the same bytes"; this answers "does anyone MEASURE what I move" — and a change with zero path overlap can still fail every test another branch owns.**

⛔ **And the resolution is never the test's threshold.** A freshly-landed effect gate exists *because* a key left the unverified list on its strength, so moving its number to make a build green is weakening a guard until a gate passes. **Either the threshold is re-derived from a new measurement with the original's published figures re-stated in the same commit, or the effect's disappearance is itself a finding and belongs in an issue.**
