...
- The grep returned **26 tracked consumers at that commit** — `matsengrp/phyz` `47a35e7c`, and **the date is part of the figure**: **17 code files across 11 directories plus 9 READMEs that state the value in prose.** ⭐ **Split the count by kind — the prose hits go stale on the edit exactly like the code hits, but they are different work, and "26 consumers" tells a worker one kind where "17 code plus 9 prose" tells it two.** ⛔ **And carry the commit with it, because this very example rotted: the same grep at `ede049ec`, three landings and ~14 hours later, returns 39 live consumers — 25 code + 14 READMEs across 15 directories — plus 2 committed results artifacts that merely CONTAIN the string and must not be edited.** ⚠ **All 13 of the new entries came from ONE of those landings, the PR whose own thesis was that this blast radius made the edit expensive: it grew the radius by 46%.** ➡ **So: a blast-radius count is a MEASUREMENT WITH A DATE, not a property of the constant. Quote the command and the commit, or do not quote the number** — and re-run it at branch time rather than inheriting any figure, including this one.
- ⛔ **A `git grep` also cannot see the INDIRECT consumers — files that import the defining module and never name the constant.** Measured at the same three commits: **14, and unchanged by any of them**, where the direct count moved 26 → 39. ⚠ **A worker who greps gets the direct set and believes it has the set.**
