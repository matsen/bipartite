...
This skill complements `/issue-check` (which reviews implementation-
ready issues for data paths, column names, test configs). `/epic-check`
...
```
/epic-check ISSUE-my-epic.md
/epic-check                     # uses most recently discussed ISSUE-*.md
```
...
`/issue-file` with the file path. EPICs are often kept as local
