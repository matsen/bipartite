...
```sh
git diff --word-diff=porcelain origin/main...HEAD -- '*.md' | grep '^-' | grep -v '^--- '
```

⚠ **Four details. The first three each produced a wrong answer the same day; the fourth was wrong in the rule itself for months.**
