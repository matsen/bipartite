...
**`bip spawn`'s behaviour comes from the binary, not from the source, so *"the source says X"* and *"the fleet does X"* are separate facts — but WHICH binary depends on how `~/go/bin/bip` is wired, and it has been wired both ways on this box.** ⛔ **Run `ls -l "$(command -v bip)"` before reasoning about either mechanism; it is one command and it tells you which one you are under.**

- **A real copy** (from `go install`). Then a landed commit changes nothing until someone reinstalls, and nothing announces the gap.
- **A symlink into the worktree.** ⚠ **This is the state on `pax` since 2026-09-19 03:47: `~/go/bin/bip -> /home/matsen/re/bipartite/bip`, an UNTRACKED build artifact (`.gitignore:13`).** Then there is no install step to lag, and the failure inverts: the fleet's `bip` is **whatever was last `go build`-ed in that worktree, from whatever branch was checked out.** It changes with no commit, it can go BACKWARDS when someone checks out an older branch and rebuilds, and **an unmerged feature branch's binary is live fleet-wide.** Measured 2026-09-21: `bip epic collide` — a subcommand on an open PR — resolved for every session on the box because the steward rebuilt from its own branch. Same shape as `~/.claude/skills/*` symlinking into that tree, one artifact over.

⭐ **The original of this rule was measured and correct, and the mechanism changed underneath it four days later.** Measured 2026-09-14: the installed binary was **18 days old** (Aug 27) when #241 landed — the commit that makes `bip spawn` pass `--name '<windowName>'` — and every behaviour-changing `bip` commit in the preceding 18 days had been live in source and absent from the fleet. That happened. It just cannot happen while the symlink is in place. ➡ **A rule about an environment needs its environment re-checked, not its wording hardened.**

⛔ **AND GREP THE BINARY FOR A SUBSTRING, NEVER FOR AN ANCHORED FLAG.** A flag that reaches `claude` through a format string never appears as a standalone string, so an anchored pattern returns **0 whether or not the behaviour is present** — a wrong zero, which reads as verified-absent and closes the question. Measured 2026-09-21 on one binary: `strings bip | grep -cE '^--name$'` → **0**, while `strings bip | grep -c "dangerously-skip-permissions --name"` → **1**, and the source is `fmt.Sprintf('claude --dangerously-skip-permissions --name \'%s\'...')` at `internal/flow/spawn/tmux.go:196`. **The behaviour was present and the check said absent.**

Before relying on a recent `bip` behaviour change in a spawn: `ls -l "$(command -v bip)"` to learn which mechanism you are under, then grep the binary for a **substring of the literal as it appears in source** — and if it is a real copy rather than a symlink, `cd ~/re/bipartite && go install ./cmd/bip` first. ⚠ **`go install` into a symlinked path is untested here and may replace the symlink with a copy, silently restoring the lag mechanism.**
